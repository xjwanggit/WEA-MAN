...
#     model.load_state_dict(torch.load(args.weight_path))
    ...
    for epoch in range(args.epochs):
        ...
        if 'Military' in args.data:
            torch.save(model.state_dict(), os.path.join(args.save_path, args.arch + '_Military_Model_water.pth'))
        elif 'CIFAR10' in args.data:
            torch.save(model.state_dict(), os.path.join(args.save_path, args.arch + '_CIFAR10_Model.pth'))

        else:
            torch.save(model.state_dict(), os.path.join(args.save_path, args.arch + '_ImageNet_100_Model.pth'))
        # save_checkpoint({
        #     'epoch': epoch + 1,
        #     'arch': args.arch,
